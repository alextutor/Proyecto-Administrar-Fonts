...
# 3. 📑 Panel de Información Detallada del Archivo
# ¿Qué hace?: Además de la vista previa, mostrar los metadatos internos e históricos de la tipografía seleccionada.
# ¿Cómo se implementaría?: Expandir el espacio de la derecha (donde está el cuadro blanco de previsualización) para detallar en etiquetas de texto:
# Peso del archivo (KB/MB).
# Formato exacto (.TTF, .OTF).
# Versión del archivo y copyright del diseñador (extrayéndolo mediante la librería PIL/Font).
# Fecha en la que fue descubierto en tu disco.
# Ademas podemos imlementar un menu ver Panel de Información Detallada del Archivo (habilitar o desabilitado (por defecto))
...
# Intentar cargar fontTools para extracción avanzada de metadatos tipográficos
try:
...
class FontManagerApp:
    def __init__(self, root):
        ...
        # Control del Panel de Información Detallada (Oculto por defecto)
        ...
        # Nuevo botón Analizar al costado del checkbox
...
""")
    ...
    def obtener_metadatos_fuente(self, path):
        ...
        info_str += (
            f"🔹 PROPIEDADES INTERNAS DE LA TIPOGRAFÍA:\n"
            f"   • Nombre comercial   : {nombre_completo}\n"
            f"   • Versión tipográfica: {version_text}\n"
            f"   • Diseñador/Creador  : {diseñador_text}\n"
    ...
    def show_details(self, tree):
        ...
        if sel:
            ...
            # Actualización en vivo del Panel de Información Detallada
            if self.show_details_panel_var.get():
    ...
    def añadir_a_coleccion(self):
        ...
        # Corregido: Centrado correcto usando el framework estandarizado del programa
        win = self._preparar_modal("Añadir a Colección", 320, 180)
        
        tk.Label(win, text="Selecciona o escribe una colección:", font=("Arial", 10)).pack(pady=10)
        ...
        def confirmar():
            nombre = cb.get().strip()
            if not nombre:
                messagebox.showerror("Error", "Debes ingresar un nombre de colección.", parent=win)
                return
            ...
            messagebox.showinfo("Éxito", f"Fuentes añadidas a '{nombre}'", parent=win)
            win.destroy()
            self.filtrar_fuentes()
