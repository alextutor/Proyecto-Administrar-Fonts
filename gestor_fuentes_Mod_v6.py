...
class FontManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestor Profesional de Fuentes - Auditoría V12")
        ...
        # Panel Superior: Lista (30%) + Preview (70%)
        top_panel = tk.Frame(paned)
        paned.add(top_panel, height=500)
        
        # Splitter Horizontal para la lista y el preview
        h_paned = tk.PanedWindow(top_panel, orient=tk.HORIZONTAL)
        h_paned.pack(fill=tk.BOTH, expand=True)

        # Lista de fuentes (30%)
        list_frame = tk.Frame(h_paned)
        h_paned.add(list_frame, width=360) # Aprox 30%
        self.tree = self.crear_tabla(list_frame, "Fuentes")
        ...
        self.tree_dup = self.crear_tabla(bot_panel, "Duplicados detectados")

        # --- Footer: Ruta seleccionada ---
    ...
    def crear_tabla(self, parent, titulo):
        tk.Label(parent, text=titulo, font=("Arial", 10, "bold")).pack(anchor="w")
        container = tk.Frame(parent)
        container.pack(fill=tk.BOTH, expand=True)
        tree = ttk.Treeview(container, columns=("Nombre", "Ruta"), show='headings')
        tree.heading("Nombre", text="Archivo"); tree.column("Nombre", width=200)
        tree.heading("Ruta", text="Ruta"); tree.column("Ruta", width=0, stretch=tk.NO)
    ...
    def preview(self, e):
        ...
        if sel:
            ...
            path = item['values'][1]
            self.selected_path_var.set(path) # Actualiza ruta al final
            try:
                img = Image.new('RGB', (800, 300), "white")
                d = ImageDraw.Draw(img)
                d.text((20, 100), "AaBbCc 123 - Previsualización", font=ImageFont.truetype(path, 60), fill="black")
                img_tk = ImageTk.PhotoImage(img)
                self.preview_lbl.config(image=img_tk, text="")
                self.preview_lbl.image = img_tk
            except: self.preview_lbl.config(text="No se pudo renderizar")
    ...
    def scan_logic(self):
        ...
        for i, f in enumerate(files):
            ...
            h = self.get_hash(f)
            if h in seen:
                self.root.after(0, lambda n=os.path.basename(f), p=f: self.tree_dup.insert("", "end", values=(n, p)))
            else: seen[h] = f
    ...
    def limpiar_duplicados(self):
        for item in self.tree_dup.get_children():
            ...
            shutil.move(path, os.path.join(self.papelera, os.path.basename(path)))
        for i in self.tree_dup.get_children(): self.tree_dup.delete(i)
        messagebox.showinfo("Limpieza", "Duplicados movidos a papelera.")
