...
#--------------Nombre Original Fuente---------------------
def obtener_nombre_real_fuente(ruta_archivo):
        """
        Lee la tabla 'name' interna de un archivo .ttf o .otf para extraer 
        el nombre real de la familia tipografica.
        """
        if not FONTTOOLS_AVAILABLE:
            # Alternativa de emergencia si fontTools fallara: usar el nombre de archivo limpio
        ...
        try:
            ...
            nombre_familia = None
            # Buscar preferentemente el nameID 1 (Family Name) o 4 (Full Name)
            for record in name_table.names:
                if record.nameID == 1:  # 1 = Font Family Name
                    # Decodificar de acuerdo a la plataforma del registro
                    if record.isUnicode():
                        nombre_familia = record.string.decode('utf-16-be').strip()
                    else:
                        nombre_familia = record.string.decode('latin-1').strip()
                    break # Rompemos en cuanto encontramos el primero optimo
                    
            # Si por alguna razon extrana el ID 1 no esta, buscamos el ID 4
            if not nombre_familia:
                for record in name_table.names:
                    if record.nameID == 4:
                        nombre_familia = record.string.decode('utf-16-be' if record.isUnicode() else 'latin-1').strip()
                        break

            font.close()
            return nombre_familia if nombre_familia else os.path.splitext(os.path.basename(ruta_archivo))[0]
        except Exception:
            # En caso de archivos corruptos o ilegibles
            return os.path.splitext(os.path.basename(ruta_archivo))[0]
...
""")
    ...
    def scan_logic(self):
        ...
        for i, f in enumerate(files):
            ...
            # ?? CAMBIO AQUI: Obtenemos el nombre real usando la funcion que agregamos antes
            nombre_b = obtener_nombre_real_fuente(f)
            
            percent = int(((i + 1) / total) * 100)
            
            if h in seen:
                # Nota: Para el duplicado se suele mostrar el nombre real o el del archivo base 
                # segun prefieras, aqui mantendra el nombre tipografico real.
