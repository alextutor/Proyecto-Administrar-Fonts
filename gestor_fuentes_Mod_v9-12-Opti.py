# Tamaño y modo claro y oscuro en la vista previa
...
class FontManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestor Profesional de Fuentes - V19.2 (Vista Previa Interactiva)")
        ...
        self.preview_dark_mode = False  # False = Claro, True = Oscuro
        self.font_size_var = tk.IntVar(value=32)  # Tamaño por defecto 32px
        ...
        tk.Label(preview_top_bar, text="Texto:", font=("Arial", 9)).pack(side=tk.LEFT, padx=(5, 2))
        self.custom_text_var = tk.StringVar()
        self.custom_text_entry = tk.Entry(preview_top_bar, textvariable=self.custom_text_var, font=("Arial", 10), width=35)
        ...
        # NUEVO: Selector Interactivo de Tamaño de Letra
        tk.Label(preview_top_bar, text="Tamaño:", font=("Arial", 9)).pack(side=tk.LEFT, padx=(10, 2))
        self.cb_size = ttk.Combobox(preview_top_bar, textvariable=self.font_size_var, values=[12, 16, 20, 24, 28, 32, 36, 42, 48, 56, 72], state="readonly", width=4)
        self.cb_size.pack(side=tk.LEFT, padx=2)
        self.cb_size.bind("<<ComboboxSelected>>", lambda e: self.show_details(self.tree))

        # NUEVO: Botón Selector de Color (Claro/Oscuro)
        self.btn_mode = tk.Button(preview_top_bar, text="☀️ Modo Claro", command=self.toggle_preview_mode, width=12, bg="#f1f5f9")
    ...
    def toggle_preview_mode(self):
        """Intercambia el esquema de color de la vista previa (Claro <-> Oscuro)"""
    ...
    def generar_preview(self, font_path):
        # Ajustamos colores dinámicamente según el modo seleccionado
        bg_color = (30, 41, 59) if self.preview_dark_mode else (255, 255, 255)     # #1e293b u blanco
        text_color = (255, 255, 255) if self.preview_dark_mode else (0, 0, 0)      # blanco o negro
        
        img = Image.new('RGB', (750, 150), color=bg_color)
        ...
        try:
            # Captura interactiva del tamaño seleccionado desde el Combobox
            font_size = int(self.font_size_var.get())
            font = ImageFont.truetype(font_path, font_size)
            
            # Dibujar el texto centrado en vertical aproximadamente
            draw.text((20, 75 - (font_size // 2)), text, fill=text_color, font=font)
        except Exception:
