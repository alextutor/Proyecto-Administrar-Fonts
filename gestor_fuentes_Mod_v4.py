...
class FontManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestor Profesional de Fuentes - Auditoría V8 (ZIP Support)")
        self.root.geometry("1200x750")
        
        self.is_running = False
        self.archivo_data = {} # {path: estado}
        self.papelera = os.path.join(os.getcwd(), "Papelera_Fuentes")
        if not os.path.exists(self.papelera): os.makedirs(self.papelera)
        
        # UI Setup (Mismo layout estable)
                ...
                with zipfile.ZipFile(f, 'r') as z:
                    for name in z.namelist():
                        if name.lower().endswith(('.ttf', '.otf')):
                            self.root.after(0, lambda n=f"{os.path.basename(f)} -> {name}": self.tree.insert("", "end", values=(n, "ZIP/CONT", f)))
            # Caso Fuente directa
            else:
    ...
    def preview(self, e):
        ...
        if path.lower().endswith('.zip'):
            self.preview_lbl.config(text="Es un archivo comprimido. Debes extraerlo para previsualizar.")
            return
